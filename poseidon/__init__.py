"""
    Poseidon is asynchronous task runner for django , it means you can run
    ...
    - >  Step 1: define your workers
        ...
        POSEIDON = {
            "poseidon_logger": "poseidon",  # the logger name which poseidon itself will be using
            "workers": {}
        }
...
def init(run_woker=True):
    ...
    global _runners
    configs = getattr(settings, "POSEIDON", {})
...
def get_runner(worker_name):
    ...
    """
    if _runners is None:
        raise RuntimeError("Poseidon didn't initialized correctly")
