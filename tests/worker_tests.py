...
from poseidon_async.workers.mpqueue import MPQueueWorker, MPQueueInitializer
from poseidon_async.workers.rabbitmq import RabbitMQWorker, RabbitMQInitializer
...
class RabbitMQWorkerTestCase(TestCase):
    ...
    def test_rabbitmq_worker_run_method(self, pika_connection):
        ...
        result_backend = {
            "result_backend_class": "poseidon_async.result_backends.dummy.DummyBackend",
        }
    ...
    @mock.patch("logging.getLogger")
    @mock.patch("poseidon_async.workers.rabbitmq.call_with_retry")
    @mock.patch("django.utils.module_loading.import_string")
...
class RabbitMQInitializerTestCase(TestCase):
    ...
    @mock.patch("poseidon_async.workers.rabbitmq.RabbitMQRunner")
...
class MPQueueWorkerTestCase(TestCase):
    ...
    @mock.patch("logging.getLogger")
    @mock.patch("poseidon_async.workers.mpqueue.call_with_retry")
    @mock.patch("django.utils.module_loading.import_string")
    @mock.patch("time.time")
...
class MPQueueInitializerTestCase(TestCase):
    ...
    @mock.patch("multiprocessing.Process")
    @mock.patch("logging.getLogger")
    @mock.patch("poseidon_async.workers.mpqueue.MPQueueWorker")
    @mock.patch("poseidon_async.workers.mpqueue.MPQueueRunner")
    def test_initializer_start_method(self, mpqr: mock.MagicMock,
        ...
        from poseidon_async.workers.mpqueue import worker
...
class CommonTestCase(TestCase):
    @mock.patch("poseidon_async.workers.common.Retrying")
    def test_call_a_function_with_some_retry_parameters(self, Retrying):
        # Arrange
        func = mock.MagicMock()
        from .common import call_with_retry as target
        retrying_params = {
            "wait_random_min": 100,
            "wait_random_max": 500,
